...
- 代理被站点拒绝（`403`/`429`/`5xx`）时同样自动轮换到下一个代理，避免"第一个代理 403 就直接失败退出"。
- 会话粘性：同一会话尽量保持同一出口 IP，仅在当前代理确实连接失败时才切换。
- 仅代理连接类错误与站点拒绝状态码触发轮换；其他站点层失败不轮换。
...
2. **触发条件为代理连接错误 + 站点拒绝状态码（403/429/5xx）**：urllib3/requests 在"连不上代理"（含代理连接超时）时抛 `ProxyError`；`403`/`429`/`5xx` 表示该代理虽连通但出口 IP 被站点/WAF 拒绝、被限流或代理上游故障，继续用该代理没有意义。其他状态码（如 `401`/`404`）与站点层业务失败不触发轮换，避免掩盖真实问题或误判站点逻辑。runner 实测（run 31257017967）确认：代理返回 403 时旧逻辑不轮换，直接 `site-unavailable` 失败退出，需要扩展触发条件。
...
- [轮换触发条件误判] → 连接类错误限定为 `ProxyError`，HTTP 拒绝限定为 `403`/`429`/`5xx`，其他状态码与异常不触发；测试覆盖正反例（403/429/5xx 轮换、401/404 不轮换）。
- [5xx 也可能是站点整体故障] → 轮换仍只遍历列表一轮，全部失败后回到现有 `site-unavailable` 语义，不会无限重试。
...
4. 手动集成：`SITE_SIJISHE_PROXY_URLS` 配置“1 个连接失败/403 代理 + 1 个好代理”，本地或 CI 试运行确认自动切换后签到成功；本地可在列表末尾加 `http://127.0.0.1:7897` 作为好代理验证自动切换；全部坏代理时确认 `site-unavailable`。runner 实测 run 31257017967 已复现旧缺陷（403 不轮换），改后需用 CI `workflow_dispatch` 复测当前代理列表。
