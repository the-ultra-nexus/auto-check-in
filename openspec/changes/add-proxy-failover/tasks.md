...
- [ ] 3.2 手动集成：`SITE_SIJISHE_PROXY_URLS` 配置“1 个坏代理 + 1 个好代理”试运行，确认自动切换后签到成功（本地可在列表末尾加 `http://127.0.0.1:7897` 作为好代理）；全部坏代理时确认 `site-unavailable`；可用 `scripts/test_proxy_connectivity.sh` 从本机或 runner 预筛代理连通性
