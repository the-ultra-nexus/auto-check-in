...
**Reason**: 静态代理列表（`CHECK_IN_PROXY_URLS` / `SITE_<NAME>_PROXY_URLS`）已被移除，代理一律来自 `CHECK_IN_PROXY_POOL_URLS` 池的按需补给。
**Migration**: 配置 `CHECK_IN_PROXY_POOL_URLS` 提供 IP 池 URL；不再支持静态列表。
