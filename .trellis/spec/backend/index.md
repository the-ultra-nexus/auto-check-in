...
| Guide | Description |
|-------|-------------|
| [Directory Structure](./directory-structure.md) | 包布局、职责边界、新增适配器流程、公共 API |
| [Configuration](./configuration.md) | 配置来源优先级、敏感键拒绝、校验模式、新增配置项清单 |
| [Error Handling](./error-handling.md) | 异常层次、状态枚举、适配器不抛业务异常、退出码 |
| [Logging Guidelines](./logging-guidelines.md) | logger 设置、级别、结构化 key=value、脱敏铁律 |
| [Network & Proxy](./network-and-proxy.md) | 直连优先、按需分批代理、失败轮换、池参数与约束 |
| [Session Cache](./session-cache.md) | cookie 会话缓存：格式、原子写、生命周期、可观测性、CI 契约 |
| [Notification Channels](./notification-channels.md) | 通道注册表、纯函数约定、SMTP 探测、新增通道流程 |
| [Quality Guidelines](./quality-guidelines.md) | 语言/工具链、代码风格、凭据安全、测试要求、禁止模式 |
